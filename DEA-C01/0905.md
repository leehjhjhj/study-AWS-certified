...
**해결책: Kinesis Data Firehose**

**분석:**

**Kinesis Data Streams + KCL 방식의 한계:**
- **추가 개발 필요**: KCL 애플리케이션 구현 및 관리
- **복잡한 아키텍처**: Streams → KCL Consumer → S3 업로드 로직
- **운영 오버헤드**: 샤드 관리, 스케일링, 에러 처리

**Kinesis Data Firehose의 장점:**
- **직접 S3 전송**: 중간 단계 없이 바로 S3로 전송
- **자동 배치 처리**: 5초 buffer 간격으로 효율적 전송
- **완전 관리형**: 인프라 관리 불필요
- **압축 및 포맷팅**: 자동 데이터 최적화

아키텍처:
```
IoT 센서 (100KB/10초) → Kinesis Data Firehose → S3 버킷
                         ↑ 5초 버퍼 설정
```
...
**왜 이 방식이 최소 지연시간인가:**
- 센서 데이터 → 5초 버퍼 → S3 = 최대 5초 지연
- Downstream이 30초마다 읽으므로 5초 지연은 충분히 빠름
- 추가 처리 단계 없어 전체 파이프라인 지연 최소화

시험 포인트:
- **IoT 데이터 → S3 직접 전송** = Kinesis Data Firehose
- **최소 지연시간 + 완전 관리형** = Firehose가 KCL보다 우수
